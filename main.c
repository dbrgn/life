...
void on_rect_button_press(ClutterActor *actor, ClutterEvent *event, gpointer data) {
    ...
    gint i = x / FIELD_WIDTH;
    gint j = y / FIELD_WIDTH;
...
void on_step_button_press(ClutterActor *actor, ClutterEvent *event, gpointer data) {
    ...
    for (i = 0; i < FIELDS_PER_SIDE; i++) {
        for (j = 0; j < FIELDS_PER_SIDE; j++) {
            color = game->fields[i][j] == 1 ? lgreen : stage_color; 
            clutter_rectangle_set_color(CLUTTER_RECTANGLE(fields[i][j]), &color);
        }
    }
}
